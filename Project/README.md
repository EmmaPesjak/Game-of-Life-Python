...
The user input was a string with the format "widthxheight". And the function needed to convert this into integers in 
...
(and later also age for the grade A implementation) was mapped to the cell dictionary. It got the neighbours 
by calling the `calc_neighbour_positions` function. Lastly the cell dictionary was mapped to the coordinates in 
...
different generation ticks. This function was later changed för grade D and B implementations.

Then the `update_world` function was implemented. An empty dictionary for the next generation was made.
A `for-loop` then iterated over the coordinates and cells in the input current generation dictionary. 
Since the rim-cells and ordinary cells have different attributes the rim-cells were taken out with an `if-statement`, 
then printed out with calling the `progress` and `get_print_value` functions from the code base with the rim-cell 
state as input. A newline was also printed after every last rim-cell of each row, using another `if-statement`, 
so that the world would not be printed out in a single line. Since the coordinates were flipped in the input dictionary
the linebreak was done at index [1] which in fact was the row coordinate. The rim cells were stored in the next 
generation dictionary and a `continue` was also put in to continue to the next cell. To print out the rest of the cells,
the `progress` and `get_print_value` functions from the code base was called with the cell state as input. 
A new cell dictionary was created for the next generation. Default values of state dead and same neighbours as in 
current generation (and later also age as 0 for the grade A implementation) was set. This was done to simplify and
shorten the `if-statement` that determined the next generation state (and age for grade A implementation).
The cell states for the next generation was determined according to the rules of transitioning stated in the 
project guidance, using an `if-statement` and calling the `count_alive_neighbours` function. The new cell dictionary
was then mapped with the coordinates in the next generation dictionary.
...
context manager and the file could be opened and stored into a variable. The data from the JSON files then needed
to be converted in order to comply with the code. The world size was converted to a tuple with the `tuple()` function.
A new population dictionary was created to store the data and conversions in. In a `for-loop` that iterated over the 
JSON file dictionary, cell coordinates were converted to tuples using `literal_eval` from the ast module.
...
age and cell state of the next generation. This was done by updating the `if-statement` that checked the 
survival requirements to include these states. In `count_alive_neighbours` instead of counting only alive neighbours
(thus technically not including elders and prime elders) the function was changed to count all not dead neighbours.
The logging information in the `wrapper_function` was also changed to include the new cell states. Like with counting
alive neighbours, since both elders and prime elders are alive, the alive counter was changed to count all not 
dead cells. The logger output was also formatted to include the added cell states.
...
its full extent, not getting a full output during development made troubleshooting harder. I depended heavily on
PyCharm's debugger to help seek out problems within the code.
...
input was "40x". The problem was solved by having the `assert-statement` sort out empty strings and raise an 
AssertionError.
...
thus getting the correct data types for the population dictionary. Once again the debugger was of great help to spot
out the differences. `literal_eval` from the ast module was new to me, but was very handy for converting the string
coordinates to the proper data type of tuples.
...
The grade A requirements were easier than I expected, and I actually found the functions for both grade E and B being 
...
significantly and thus improving readability. Going through the code again, I also noticed I was prematurely ageing 
my newborn cells in the `update_world` function, setting their age to one in their first alive generation when it 
should be zero.
